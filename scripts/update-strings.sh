...
do
  TRANSLATION_FILE="i18n/QGisImajnetPlugin_$LOCALE.ts"
...
then
  ...
  for LOCALE in ${LOCALES}
  do
    echo "i18n/QGisImajnetPlugin_"${LOCALE}".ts"
    # Note we don't use pylupdate with qt .pro file approach as it is flakey
    # about what is made available.
    pylupdate4 -noobsolete ${PYTHON_FILES} -ts i18n/QGisImajnetPlugin_${LOCALE}.ts
  done
else
