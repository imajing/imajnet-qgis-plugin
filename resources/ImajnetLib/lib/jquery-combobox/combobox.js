Combobox = {
    defaultOptions: {
        _create: function() {
	        var input, self = this, select = this.element.hide(), selected = select.children(":selected"), value = selected.val() ? selected.text() : "", wrapper = this.wrapper = jQuery("<span>")
	            .attr('id', jQuery(this.element).attr('id') + 'Combobox').addClass("ui-combobox ui-combobox-" + jQuery(this.element).attr('id')).insertAfter(select), firstOption = null;
	        input = jQuery("<input>").appendTo(wrapper).val(value).addClass('ui-state-default ui-combobox-input' + (self.options.permisive ? ' ui-combobox-input-permisive' : ' ui-combobox-input-nopermisive'))
	            .autocomplete({
	                delay: 0,
	                minLength: 0,
	                ...
	                change: function(event, ui) {
		                ...
		                if(!ui.item) {
			                var matcher = new RegExp("^" + jQuery.ui.autocomplete.escapeRegex(jQuery(this).val()) + "$", "i"), valid = false;
			                ...
			                if(!valid) {
				                ...
				                if(select.children("option")[0] && select.children("option")[0].value) {
					                ...
					                firstOptionText = select.children("option")[0].text;
				                }

				                jQuery(this).val(firstOptionText);
	        ...
	        input.data("autocomplete")._renderItem = function(ul, item) {
		        ...
		        ul.on('mouseleave', function(event) {
			        if(event && event.currentTarget && event.currentTarget.className &&
			            (event.currentTarget.className.indexOf('imajnetSettingsMenuSelectOptions') !== -1 || event.currentTarget.className.indexOf('imajnetSettingsMenuLocaleSelectOptions') !== -1)) {
				        if(Nigsys.getCookie('IMAJNET', 'DOCKING_header') == 'false') {
					        ImajnetUI.docking.header.collapseTopContainer({
					            isTriggered: true,
					            target: {},
					            data: {
						            type: 'header'
    ...
    permisiveOptions: {
        _create: function() {
	        var input, self = this, select = this.element.hide(), selected = select.children(":selected"), value = selected.val() ? selected.text() : "", wrapper = this.wrapper = jQuery("<span>").addClass("ui-combobox")
	            .insertAfter(select);
